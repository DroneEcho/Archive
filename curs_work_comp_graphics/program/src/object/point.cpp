...
point4::point4(double x, double y, double z, double f)
    :x(x), y(y), z(z), f(f)
{
}

double point4::get_x() const
{
    return x;
}

void point4::set_x(double x)
{
    this->x = x;
}

double point4::get_y() const
{
    return y;
}

void point4::set_y(double y)
{
    this->y = y;
}

double point4::get_z() const
{
    return z;
}

void point4::set_z(double z)
{
    this->z = z;
}

double point4::get_f() const
{
    return f;
}

void point4::set_f(double f)
{
    this->f = f;
}

void point4::move(const point4 &move)
{
    x += move.x;
    y += move.y;
    z += move.z;
...
void point4::scale(const point4 &center, const point4 &scale)
{
    x = x * scale.x + (1 - scale.x) * center.x;
    y = y * scale.y + (1 - scale.y) * center.y;
    z = z * scale.z + (1 - scale.z) * center.z;
...
void point4::rotate_xy(double angle)
{
    double xy_angle = (angle * M_PI) / 180;
...
void point4::rotate_xz(double angle)
{
    double xz_angle = (angle * M_PI) / 180;
...
void point4::rotate_yz(double angle)
{
    double yz_angle = (angle * M_PI) / 180;
...
void point4::rotate(const point4 &center, const point4 &rotate)
{
    x -= center.x;
    y -= center.y;
    z -= center.z;
