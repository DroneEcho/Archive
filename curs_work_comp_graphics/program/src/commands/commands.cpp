...
void load_existingModel_command::execute() const
{
    if (!scene_manager_creator().get_manager()->is_camera_set())
        throw no_camera_exception();
    ...
    const std::map<std::string, std::string> files =
        {
            {"Куб", "/C/Users/Maria/Desktop/cursprogdo/curs/data/cube_new.obj"},
            {"Сфера", "/C/Users/Maria/Desktop/cursprogdo/curs/data/sphere.obj"},
            {"Конус", "/C/Users/Maria/Desktop/cursprogdo/curs/data/conus.obj"},
            {"Цилиндр", "/C/Users/Maria/Desktop/cursprogdo/curs/data/cylinder.obj"}
        };
    ...
    std::shared_ptr<object> _model_obj(new model_poligonal(files.at(name), uid, n_power.at(name)));

    scene_manager_creator().get_manager()->get_scene()->add_new_model(_model_obj);
    //scene_manager_creator().get_manager()->get_scene()->new_models->add(_model_obj);
    //scene_manager_creator().get_manager()->get_scene()->add_new_model();

    //new_models.push_back(new model_poligonal(files.at(name), uid, n_power.at(name)));
    //scene_manager_creator().get_manager()->get_scene()->add_model(_model_obj);
    //std::shared_ptr<object> _model_obj = _lm->load(file_name);
    //scene_manager_creator().get_manager()->get_scene()->add_model(_model_obj);
}
...
{
    std::shared_ptr<object> _camera(new camera(pos, w, h));
