#include <QMessageBox>
#include <QString>
#include <QFileDialog>

#include "mainwindow.h"
#include "ui_mainwindow.h"
#include "base_exception.h"
#include "ui_exceptions.h"
#include "commands.h"
#include "abstract_factory.h"
#include "qt_factory.h"
#include "point.h"
#include "camera.h"

UI_data::UI_data(bool isLight_, const point& p){
    img.load("/C/Users/Maria/Desktop/cursprogdo/curs/data/textures/bricks.jpg");
    isLight = isLight_;
    shift_x = p.get_x(), shift_y = p.get_y(), shift_z = p.get_z();
}

MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::MainWindow)
{
    ui->setupUi(this);
    setup_scene();
}

MainWindow::~MainWindow()
{
    delete ui;
}

void MainWindow::setup_scene()
{
    _scene = std::shared_ptr<QGraphicsScene>(new QGraphicsScene(this));
    ui->canvas->setScene(_scene.get());
    ui->canvas->setAlignment(Qt::AlignTop | Qt::AlignLeft);
    _scene->setSceneRect(0, 0, ui->canvas->width(), ui->canvas->height());

    std::shared_ptr<abstract_factory> factory(new qt_factory(_scene));
    _drawer = factory->create_drawer();

    const QStringList figures =
    {
        "Куб",
        "Параллелепипед",
        "Цилиндр",
        "Сфера",
        "Пирамида"
    };
    ui->modelsList->addItems(figures);

    //init cam default

    on_addCameraBtn_clicked();

    auto stringList = new QStringList();
    model = new QStringListModel(*stringList);

    ui->scene_list->setModel(model);
    ui->scene_list->setEditTriggers(QAbstractItemView::NoEditTriggers);

    connect(ui->scene_list, SIGNAL(clicked(QModelIndex)), this, SLOT(fetch(QModelIndex)));
    ui->color_preview->setScene(new QGraphicsScene);
    //ui->color_view->setScene(new QGraphicsScene);
    //ui->color_view_2->setScene(new QGraphicsScene);
}

void MainWindow::render_scene()
{
    std::shared_ptr<base_command> com(new draw_scene_command(_drawer));

    try
    {
        _facade.execute_command(com);
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::on_addCameraBtn_clicked()
{
    auto id = std::make_shared<size_t>(0);
    point location ( -800.0, -500.0, 10.0);
    auto w = ui->canvas->width();
    auto h = ui->canvas->height();
    std::shared_ptr<base_command> com(new add_camera_command(location, w, h));

    _facade.execute_command(com);

    std::shared_ptr<base_command> com1(new set_camera_command(cams_count));
    _facade.execute_command(com1);

    auto cam = ui->cameraCB;

    std::string camName = std::string("Camera ") + std::to_string(cams_count++);
    cam->addItem(QString(camName.data()));

    ui->cameraCB->setCurrentIndex(ui->cameraCB->count() - 1);

    render_scene();
}

void MainWindow::on_cameraCB_currentIndexChanged(int index)
{
    std::shared_ptr<base_command> com(new set_camera_command(index));

    try
    {
        _facade.execute_command(com);

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::on_loadModelBtn_clicked()
{
    auto file = QFileDialog::getOpenFileName(nullptr, "Загрузка модели", "../curs/data/cube.txt");
    std::shared_ptr<base_command> com(new load_command(file.toUtf8().data()));

    try
    {
        _facade.execute_command(com);
        ui->modelsCB->addItem(file);

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::on_deleteCameraBtn_clicked()
{
    std::shared_ptr<base_command> com(new remove_camera_command(ui->cameraCB->currentIndex()));

    try
    {
        _facade.execute_command(com);
        ui->cameraCB->removeItem(ui->cameraCB->currentIndex());
        cams_count--;
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void on_NewFile_clicked()
{

}

void on_SaveFile_clicked()
{

}
void MainWindow::on_addModel_clicked()
{
    size_t index = ui->modelsList->currentIndex();
    auto name = ui->Namemodel->text();
    auto group = ui->GroupModel->text(); ///!!!!!!

    if (name.isEmpty())
        name = ui->modelsList->currentText();

    auto updated_name = name;

    if (!obj_data.count(name))
        obj_data.insert({name, UI_data{}});
    else
    {
        auto val = ++obj_data.at(name).amount;
        updated_name += QString("%1").arg(val);
        obj_data.insert({updated_name, UI_data{}});
    }

    uint32_t uid = 0;

    auto text = ui->modelsList->currentText().toStdString();
    std::shared_ptr<base_command> com(new load_existingModel_command(text, uid, models_index));
    _facade.execute_command(com);

    models_index++;

    //manager.uploadModel(text.toStdString(), uid);

    if (uid)
    {
        model->insertRow(model->rowCount());
        QModelIndex index = model->index(model->rowCount()-1);
        model->setData(index, updated_name);
        text_uid.insert({updated_name, uid});
    }
}
void on_push_backLight_clicked()
{

}
void on_changeColourLight_clicked()
{

}
void on_delete_object_button_clicked()
{

}
void on_push_back_texture_button_clicked()
{

}
void on_color_push_back_button_clicked()
{

}


void on_ViewColour_clicked()
{

}
void on_Render_clicked()
{

}

void MainWindow::on_upBtn_clicked()
{
    try
    {
        point move(0.0, -10.0, 0.0);
        size_t index = ui->cameraCB->currentIndex();
        std::shared_ptr<base_command> com(new move_camera_command(index, move));

        _facade.execute_command(com);

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}


void MainWindow::on_downBtn_clicked()
{
    try
    {
        point move(0.0, 10.0, 0.0);
        size_t index = ui->cameraCB->currentIndex();
        std::shared_ptr<base_command> com(new move_camera_command(index, move));

        _facade.execute_command(com);

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::on_rigthBtn_clicked()
{
    try
    {
        point move(10.0, 0.0, 0.0);
        size_t index = ui->cameraCB->currentIndex();
        std::shared_ptr<base_command> com(new move_camera_command(index, move));

        _facade.execute_command(com);

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::on_rightUpBtn_clicked()
{
    try
    {
        point move(10.0, -10.0, 0.0);
        size_t index = ui->cameraCB->currentIndex();
        std::shared_ptr<base_command> com(new move_camera_command(index, move));

        _facade.execute_command(com);

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::on_rigntDownBtn_clicked()
{
    try
    {
        point move(10.0, 10.0, 0.0);
        size_t index = ui->cameraCB->currentIndex();
        std::shared_ptr<base_command> com(new move_camera_command(index, move));

        _facade.execute_command(com);

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::on_leftBtn_clicked()
{
    try
    {
        point move(-10.0, 0.0, 0.0);
        size_t index = ui->cameraCB->currentIndex();
        std::shared_ptr<base_command> com(new move_camera_command(index, move));

        _facade.execute_command(com);

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::on_leftUpBtn_clicked()
{
    try
    {
        point move(-10.0, -10.0, 0.0);
        size_t index = ui->cameraCB->currentIndex();
        std::shared_ptr<base_command> com(new move_camera_command(index, move));

        _facade.execute_command(com);

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::on_leftDownBtn_clicked()
{
    try
    {
        point move(-10.0, 10.0, 0.0);
        size_t index = ui->cameraCB->currentIndex();
        std::shared_ptr<base_command> com(new move_camera_command(index, move));

        _facade.execute_command(com);

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::on_moveBtn_clicked()
{
    try
    {
        double dx, dy, dz;
        dx = ui->dxDSB->value();
        dy = ui->dyDSB->value();
        dz = ui->dzDSB->value();
        point move(dx, dy, dz);
        size_t index = ui->cameraCB->currentIndex();
        std::shared_ptr<base_command> com(new move_camera_command(index, move));

        _facade.execute_command(com);

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::on_rotateBtn_clicked()
{
    try
    {
        double ox, oy, oz;
        ox = ui->oxDSB->value();
        oy = ui->oyDSB->value();
        oz = ui->ozDSB->value();
        point rotate(ox, oy, oz);
        size_t index = ui->cameraCB->currentIndex();
        std::shared_ptr<base_command> com(new rotate_camera_command(index, rotate));

        _facade.execute_command(com);

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void on_loadModelBtn_clicked()
{

}
void on_deleteModelBtn_clicked()
{

}

/*
void MainWindow::on_load_mod_but_clicked()
{
    auto file = QFileDialog::getOpenFileName();
    std::shared_ptr<base_command> com(new load_command(file.toUtf8().data()));

    try
    {
        _facade.execute_command(com);
        ui->model_pick->push_backItem(file);

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::on_remove_mod_but_clicked()
{
    std::shared_ptr<base_command> com(new remove_model_command(ui->model_pick->currentIndex()));

    try
    {
        _facade.execute_command(com);
        ui->model_pick->removeItem(ui->model_pick->currentIndex());

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::on_push_back_cam_but_clicked()
{
    auto file = QFileDialog::getOpenFileName();
    std::shared_ptr<base_command> com(new load_camera_command(file.toUtf8().data()));

    try
    {
        _facade.execute_command(com);
        ui->cam_pick->push_backItem(file);
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::on_set_cam_but_clicked()
{
    std::shared_ptr<base_command> com(new set_camera_command(ui->cam_pick->currentIndex()));

    try
    {
        _facade.execute_command(com);

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::on_remove_cam_but_clicked()
{
    std::shared_ptr<base_command> com(new remove_camera_command(ui->cam_pick->currentIndex()));

    try
    {
        _facade.execute_command(com);
        ui->cam_pick->removeItem(ui->cam_pick->currentIndex());
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::scan_move_model_params(double &dx, double &dy, double &dz) const
{
    bool is_double;

    dx = ui->mod_dx_inp->text().toDouble(&is_double);
    if (!is_double)
        throw ui_field_format_exception();

    dy = ui->mod_dy_inp->text().toDouble(&is_double);
    if (!is_double)
        throw ui_field_format_exception();

    dz = ui->mod_dz_inp->text().toDouble(&is_double);
    if (!is_double)
        throw ui_field_format_exception();
}

void MainWindow::scan_scale_model_params(double &kx, double &ky, double &kz) const
{
    bool is_double;

    kx = ui->mod_kx_inp->text().toDouble(&is_double);
    if (!is_double)
        throw ui_field_format_exception();

    ky = ui->mod_ky_inp->text().toDouble(&is_double);
    if (!is_double)
        throw ui_field_format_exception();

    kz = ui->mod_kz_inp->text().toDouble(&is_double);
    if (!is_double)
        throw ui_field_format_exception();
}

void MainWindow::scan_rotate_model_params(double &ox, double &oy, double &oz) const
{
    bool is_double;

    ox = ui->mod_ox_inp->text().toDouble(&is_double);
    if (!is_double)
        throw ui_field_format_exception();

    oy = ui->mod_oy_inp->text().toDouble(&is_double);
    if (!is_double)
        throw ui_field_format_exception();

    oz = ui->mod_oz_inp->text().toDouble(&is_double);
    if (!is_double)
        throw ui_field_format_exception();
}

void MainWindow::on_move_mod_but_clicked()
{
    try
    {
        double dx, dy, dz;
        scan_move_model_params(dx, dy, dz);
        point move(dx, dy, dz);
        size_t index = ui->model_pick->currentIndex();
        std::shared_ptr<base_command> com(new move_model_command(index, move));

        _facade.execute_command(com);

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::on_scale_mod_but_clicked()
{
    try
    {
        double kx, ky, kz;
        scan_scale_model_params(kx, ky, kz);
        point scale(kx, ky, kz);
        size_t index = ui->model_pick->currentIndex();
        std::shared_ptr<base_command> com(new scale_model_command(index, scale));

        _facade.execute_command(com);

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::on_rotate_mod_inp_clicked()
{
    try
    {
        double ox, oy, oz;
        scan_rotate_model_params(ox, oy, oz);
        point rotate(ox, oy, oz);
        size_t index = ui->model_pick->currentIndex();
        std::shared_ptr<base_command> com(new rotate_model_command(index, rotate));

        _facade.execute_command(com);

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::scan_move_camera_params(double &dx, double &dy, double &dz) const
{
    bool is_double;

    dx = ui->cam_dx_inp->text().toDouble(&is_double);
    if (!is_double)
        throw ui_field_format_exception();

    dy = ui->cam_dy_inp->text().toDouble(&is_double);
    if (!is_double)
        throw ui_field_format_exception();

    dz = ui->cam_dz_inp->text().toDouble(&is_double);
    if (!is_double)
        throw ui_field_format_exception();
}

void MainWindow::scan_rotate_camera_params(double &ox, double &oy, double &oz) const
{
    bool is_double;

    ox = ui->cam_ox_inp->text().toDouble(&is_double);
    if (!is_double)
        throw ui_field_format_exception();

    oy = ui->cam_oy_inp->text().toDouble(&is_double);
    if (!is_double)
        throw ui_field_format_exception();

    oz = ui->cam_oz_inp->text().toDouble(&is_double);
    if (!is_double)
        throw ui_field_format_exception();
}

void MainWindow::on_move_cam_but_clicked()
{
    try
    {
        double dx, dy, dz;
        scan_move_camera_params(dx, dy, dz);
        point move(dx, dy, dz);
        size_t index = ui->cam_pick->currentIndex();
        std::shared_ptr<base_command> com(new move_camera_command(index, move));

        _facade.execute_command(com);

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}

void MainWindow::on_rotate_cam_btn_clicked()
{
    try
    {
        double ox, oy, oz;
        scan_rotate_camera_params(ox, oy, oz);
        point rotate(ox, oy, oz);
        size_t index = ui->cam_pick->currentIndex();
        std::shared_ptr<base_command> com(new rotate_camera_command(index, rotate));

        _facade.execute_command(com);

        render_scene();
    }
    catch (base_exception &ex)
    {
        QMessageBox::warning(this, "Error", QString(ex.what()));
    }
}
*/
