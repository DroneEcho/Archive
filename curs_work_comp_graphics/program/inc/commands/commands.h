...
{
public:
    explicit add_camera_command(const point &pos, float w, float h) : pos(pos), w(w), h(h) {}
...
class load_existingModel_command : public base_command
{
public:
    load_existingModel_command(std::string name, uint32_t& uid, uint32_t newuid) : name(name), uid(uid), newuid(newuid) {}
